...
async function init(){
  ...
  document.getElementById('clearFilters').addEventListener('click', ()=>{
    ...
    filtered=articles.slice();render();
  });
}
...
function render(){
  ...
  for(const a of pageItems){
    ...
    const card=document.createElement('div');card.className='card news-card h-100 shadow-sm';
    const img=document.createElement('img');img.src=a.thumb||'/placeholder.png';img.className='card-img-top thumb';img.alt=a.title;
    const body=document.createElement('div');body.className='card-body d-flex flex-column';
    const h5=document.createElement('h5');h5.className='card-title';h5.textContent=a.title;
    const p=document.createElement('p');p.className='card-text text-muted flex-grow-1';p.textContent=a.excerpt||'';
    const meta=document.createElement('div');meta.className='d-flex justify-content-between align-items-center mt-3';
    const small=document.createElement('small');small.className='text-muted';small.textContent=(a.date||'')+' • '+(a.category||'');
    const btn=document.createElement('button');btn.className='btn btn-sm btn-primary';btn.textContent='Baca';
    btn.addEventListener('click',()=>openArticle(a));
    meta.appendChild(small);meta.appendChild(btn);
    body.appendChild(h5);body.appendChild(p);body.appendChild(meta);
    card.appendChild(img);card.appendChild(body);col.appendChild(card);list.appendChild(col);
  }
  renderPagination();
}
...
async function openArticle(a){
  ...
  document.getElementById('modalImage').src=a.thumb||'/placeholder.png';
  document.getElementById('modalDate').textContent=a.date||'-';
  document.getElementById('modalCategory').textContent=a.category||'-';
  document.getElementById('modalBodyText').textContent='Memuat...';

  const url=a.txt || `/news/${a.slug}/text.txt`;
  try{
    ...
    document.getElementById('modalBodyText').innerHTML=
      `<div class="alert alert-info">Isi berita tidak ditemukan pada ${url}.</div><p>${escapeHtml(a.excerpt||'')}</p>`;
  }
  modal.show();
}
